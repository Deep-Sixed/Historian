...
CORPUS = Path(os.environ.get(
    "HISTORIAN_CORPUS",
    "/mnt/jarvis-data/projects/chatgpt-export/corpus",
))
SECRET = Path("/mnt/jarvis-data/projects/EVECOR/bin/jarvis-secret")
DOC = os.environ.get("HISTORIAN_TEST_DOC", "2026-07-08-decommissioning-hindsight-memory")
...
def _secret(name):
    ...
    if not SECRET.exists():
        if os.environ.get("HISTORIAN_RUN_PG") == "1":
            pytest.fail("jarvis-secret unavailable and no environment credential supplied")
...
def test_wrong_document_fails_closed(reader, real):
    ...
    cid = propose(rid("C-wrongdoc"), source_id="2026-07-14-hindsight-v0-8-4-review",
