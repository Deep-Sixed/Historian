...
Credentials come from redacted via redacted-secret. There is no credential file.
"""
...
# Vault entry title per database role. Credentials live ONLY in redacted; there is no
...
def test_human_reviewer_can_write_a_reviewed_proposal(seeded, packets):
    with conn("historian_human_reviewer") as c:
        c.execute("""INSERT INTO asserted_relation
            ...
            VALUES (%s,'e1','AUGMENTS','e2','HUMAN_REVIEWED_PROPOSAL','redacted','P1')""",
                  (rid("A-rev"),))
