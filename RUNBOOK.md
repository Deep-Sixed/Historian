...
## Database
    ...
    container   redacted-historian-db     postgres:18-alpine, --memory 1g, restart no
...
Provision from scratch:
    ...
    docker run -d --name redacted-historian-db \
    ...
    docker exec -i redacted-historian-db psql -U historian_owner -d evecor_historian \
...
Use `sql/provision.sh`, which reads every credential from redacted. It exists because two
...
normal way:
    ...
    redacted-secret get HISTORIAN_RUNTIME

Wired into `bin/redacted-secret` (`APPROVED_NAMES`) and `bin/keepass_batch.py`
...
test suite reads exclusively through `redacted-secret`.
...
`historian_adjudicator` is a GROUP role with **no login**. Each adjudicating human needs:
    ...
    CREATE ROLE adj_<human> LOGIN PASSWORD '<from redacted>';
...
redacted record. HIST-C3 cannot supply it: its verdict is already in the RAG v1 manifest
