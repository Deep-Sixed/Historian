...
* HIST-C3 remains a regression exhibit, not gold: its verdict already exists in the RAG v1
  manifest and ledger `clm-2026-eb52f4cd`, so no one with access to the redacted record can
  adjudicate it blind.
...
redacted found the next hole using **my own test fixture as the proof**: seed S1 selects
...
Probed at redacted's instruction rather than redesigned. The full chain **succeeds**:
...
redacted's ruling, and the right one: `EvidenceRef` already *means* "a source-backed,
...
**22. `derived_from_candidate_id` proved existence, not realisation.** Probed at redacted's
